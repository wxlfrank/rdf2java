...
public class JavaGenerator {
	...
	public void generate(File location, Collection<Binding> packages) {
		...
		packages.forEach(binding -> {
			PackageEx packex = (PackageEx) binding;
			...
			if (packex != Constants.XMLSchema_Package) {
				...
				write2File(pac_dir, PACKAGE_HEADER,
						String.format(PACKAGE_RDF_ANNOTATION, pac.getName()) + package_header + RDF_ANNOTATION_IMPORT);
	...
	private String addImport(Type type) {
		if (type == null)
			return "";
		String str = config.getPackageName(type.getContainer().getName());
		if (!str.isEmpty())
			str += ".";
		str += type.getName();
			...
			types.forEach(type -> {
				if (!result.containsKey(type)) {
					if (type.getContainer().getName().equals(Constants.XMLSchema_URI)) {
						...
						result.put(type,
								config.getPackageName(type.getContainer().getName()) + "." + type.getName());
				}
			});
		});
		return result;
	...
	protected String toJavaFieldStatement(Map<Type, String> type2name, Field field) {
		...
		return String.format(FIELD_FORMAT, type == null ? null : type2name.get(type), name,
				Utils.captializeFirstChar(name), pack.getName(), field.getName());
